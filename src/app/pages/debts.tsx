...
import { mockDebts, mockAccounts, mockTransactions, formatCurrency, daysUntil } from '../store';
...
export function Debts() {
  ...
  const lentDebts = mockDebts.filter(d => d.type === 'lent');
  const borrowedDebts = mockDebts.filter(d => d.type === 'borrowed');
  ...
  return (
    <div className="max-w-[1600px] mx-auto">
      ...
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-[#3D4852] text-3xl mb-2">Debts & Spaylater</h1>
                ...
                return (
                  <div key={debt.id} className="border-b border-[#CDD2D9]/30 last:border-0 pb-4 last:pb-0">
                    <div className="flex items-start justify-between mb-4">
                      <div className="flex items-center gap-4">
                        <div className="w-14 h-14 bg-[#E0E5EC] rounded-2xl shadow-[inset_4px_4px_8px_rgba(163,177,198,0.6),inset_-4px_-4px_8px_rgba(255,255,255,0.6)] flex items-center justify-center">
                          <CreditCard size={24} className="text-[#FF6B6B]" />
                        </div>
                        <div>
                          <p className="text-[#3D4852] text-lg mb-1">{debt.person}</p>
                          <p className="text-[#8B92A0] text-sm">{debt.description}</p>
        ...
        >
          ...
          >
            <NeumorphicCard className="p-6">
              ...
              {/* Type Selection */}
              <div className="mb-6">
                <label className="block text-[#3D4852] mb-3">Type</label>
                  ...
                  {(Object.keys(debtTypeConfig) as Array<'lent' | 'borrowed' | 'installments'>).map((type) => {
                    ...
                    return (
                      ...
                      >
                        <Icon 
                          size={24} 
                          className={`mx-auto mb-2 ${debtType === type ? 'text-white' : 'text-[#3D4852]'}`}
                ...
                <p className="text-[#8B92A0] text-sm mt-2">{debtTypeConfig[debtType].description}</p>
              </div>

              {/* Person/Organization */}
              <div className="mb-6">
                <label className="block text-[#3D4852] mb-2">
                  {debtType === 'installments' ? 'Store/Company' : 'Person Name'}
                </label>
                <NeumorphicInput
                  type="text"
                  placeholder={debtType === 'installments' ? 'FE Credit, Home Credit...' : 'John Doe, Jane Smith...'}
                  value={person}
              ...
              {/* Total Amount */}
              <div className="mb-6">
                <label className="block text-[#3D4852] mb-2">Total Amount (VND)</label>
                <NeumorphicInput
                  type="number"
                  placeholder="0"
                  value={totalAmount}
              ...
              <div className="mb-6">
                <label className="block text-[#3D4852] mb-2">
                  {debtTypeConfig[debtType].amountLabel} (VND)
                </label>
                <NeumorphicInput
                  type="number"
                  placeholder="0"
              ...
              {/* Due Date */}
              <div className="mb-6">
                <label className="block text-[#3D4852] mb-2">Due Date (Optional)</label>
                <NeumorphicInput
                  type="date"
                  value={dueDate}
              ...
              {totalAmount && Number(totalAmount) > 0 && (
                <div className="mb-6 p-4 bg-[#E0E5EC] rounded-2xl shadow-[inset_2px_2px_4px_rgba(163,177,198,0.4),inset_-2px_-2px_4px_rgba(255,255,255,0.4)]">
                  ...
                  <div className="flex items-center gap-4 mb-4">
                    ...
                    >
                      {(() => {
                        ...
                        return <Icon size={24} style={{ color: debtTypeConfig[debtType].color }} />;
                      })()}
                    </div>
                    <div className="flex-1">
                      <p className="text-[#3D4852]">{person || 'Person Name'}</p>
                  ...
                  <div className="grid grid-cols-2 gap-3 mb-3">
                    <div>
                      <p className="text-[#8B92A0] text-xs mb-1">
                        {debtTypeConfig[debtType].amountLabel}
                      </p>
                      <p className="text-[#4ECDC4] text-lg">{formatCurrency(Number(paidAmount) || 0)}</p>
                    </div>
                    <div>
                      <p className="text-[#8B92A0] text-xs mb-1">Remaining</p>
                      <p className="text-[#FF6B6B] text-lg">
                ...
                <div>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <NeumorphicButton 
                      ...
                      onClick={() => {
                      if (selectedDebt && receiveAmount && Number(receiveAmount) > 0) {
                        const receiveAmountNum = Number(receiveAmount);
                        
                        // 1. Create Income Transaction (if account selected)
                        if (receiveAccount) {
                          const newTransaction = {
                            ...
                            description: `Received from ${selectedDebt.person} - ${selectedDebt.description}${receiveNote ? ` (${receiveNote})` : ''}`
                          };
                          mockTransactions.push(newTransaction);
                          
                          // 2. Update Account Balance
                          const account = mockAccounts.find(a => a.id === receiveAccount);
                          if (account) {
                            account.balance += receiveAmountNum;
                          }
                          
                          console.log('✅ Created Income Transaction:', newTransaction);
                          console.log('✅ Updated Account Balance:', account);
                        }
                        
                        // 3. Update Debt
                        const newProgress = ((selectedDebt.amount + receiveAmountNum) / selectedDebt.totalAmount) * 100;
                        
                        // 4. Add to Payment History
                        if (!selectedDebt.paymentHistory) {
                          selectedDebt.paymentHistory = [];
                        }
                        selectedDebt.paymentHistory.push({
                          id: `p${Date.now()}`,
                          date: receiveDate,
                          amount: receiveAmountNum,
                        ...
                        selectedDebt.amount += receiveAmountNum;
                        
                        console.log('✅ Updated Debt:', selectedDebt);
                      ...
                      {receiveAccount && (
                        <div className="mt-4 p-3 bg-[#4ECDC4]/10 rounded-xl border-2 border-[#4ECDC4]/30">
                          ...
                          <div className="space-y-1 text-sm">
                            ...
                            <p className="text-[#8B92A0]">• Account: <span className="text-[#3D4852] font-medium">{mockAccounts.find(a => a.id === receiveAccount)?.name}</span></p>
                ...
                <div>
                  ...
                  <div className="mb-5">
                    ...
                    {payAccount && payAmount && Number(payAmount) > 0 && (
                      <>
                        {(() => {
                          const selectedAccount = mockAccounts.find(a => a.id === payAccount);
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <NeumorphicButton 
                      ...
                      onClick={() => {
                      if (selectedBorrowedDebt && payAmount && Number(payAmount) > 0) {
                        const payAmountNum = Number(payAmount);
                        
                        // 1. Create Expense Transaction (if account selected)
                        if (payAccount) {
                          const newTransaction = {
                            id: (mockTransactions.length + 1).toString(),
                            ...
                            description: `Payment to ${selectedBorrowedDebt.person} - ${selectedBorrowedDebt.description}${payNote ? ` (${payNote})` : ''}`
                          };
                          mockTransactions.push(newTransaction);
                          
                          // 2. Update Account Balance (subtract)
                          const account = mockAccounts.find(a => a.id === payAccount);
                          if (account) {
                            account.balance -= payAmountNum;
                          }
                          
                          console.log('✅ Created Expense Transaction:', newTransaction);
                          console.log('✅ Updated Account Balance:', account);
                        }
                        
                        // 3. Update Debt
                        const newProgress = ((selectedBorrowedDebt.amount + payAmountNum) / selectedBorrowedDebt.totalAmount) * 100;
                        
                        // 4. Add to Payment History
                        if (!selectedBorrowedDebt.paymentHistory) {
                          selectedBorrowedDebt.paymentHistory = [];
                        }
                        selectedBorrowedDebt.paymentHistory.push({
                          id: `p${Date.now()}`,
                          date: payDate,
                          amount: payAmountNum,
                          note: payNote || 'Payment made',
                          progressAtTime: Math.round(newProgress)
                        });
                        
                        selectedBorrowedDebt.amount += payAmountNum;
                        
                        console.log('✅ Updated Debt:', selectedBorrowedDebt);
                      ...
                      {payAccount && (
                        <div className="mt-4 p-3 bg-[#FF6B6B]/10 rounded-xl border-2 border-[#FF6B6B]/30">
                          ...
                          <div className="space-y-1 text-sm">
                            ...
                            <p className="text-[#8B92A0]">• Account: <span className="text-[#3D4852] font-medium">{mockAccounts.find(a => a.id === payAccount)?.name}</span></p>
