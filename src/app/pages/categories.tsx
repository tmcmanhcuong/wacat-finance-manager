    ...
    return (
      <div className="flex items-center justify-center h-96">
        <div className="flex flex-col items-center gap-4">
          ...
          <p className="text-[#8B92A0]">Loading categories...</p>
        </div>
      </div>
    );
  }
              ...
              <NeumorphicCard className="p-6 sticky top-8">
                ...
                <div className="grid grid-cols-2 gap-3 mb-6">
                  <button
                    ...
                    className={`py-4 rounded-2xl transition-all min-h-[56px] ${categoryType === 'income' ? 'bg-[#4ECDC4] text-white' : 'bg-[#E0E5EC] text-[#3D4852] shadow-[4px_4px_8px_rgba(163,177,198,0.6),-4px_-4px_8px_rgba(255,255,255,0.6)]'}`}
                  >
                    <TrendingUp size={18} className="inline mr-2" />Income
                  </button>
                  <button
                    onClick={() => setCategoryType('expense')}
                    className={`py-4 rounded-2xl transition-all min-h-[56px] ${categoryType === 'expense' ? 'bg-[#FF6B6B] text-white' : 'bg-[#E0E5EC] text-[#3D4852] shadow-[4px_4px_8px_rgba(163,177,198,0.6),-4px_-4px_8px_rgba(255,255,255,0.6)]'}`}
                  >
                      ...
                      {availableIcons.map(icon => {
                        ...
                        return (
                          <button
                            ...
                            className={`w-full aspect-square rounded-xl flex items-center justify-center transition-all ${selectedIcon === icon.name ? 'bg-[#6C63FF] shadow-[4px_4px_8px_rgba(108,99,255,0.4)]' : 'bg-[#E0E5EC] shadow-[2px_2px_4px_rgba(163,177,198,0.4),-2px_-2px_4px_rgba(255,255,255,0.4)]'}`}
                          >
                            <IconComp size={20} className={selectedIcon === icon.name ? 'text-white' : 'text-[#8B92A0]'} />
                          </button>
                        );
                      })}
                    </div>
                ...
                <div className="mb-6">
                  ...
                  <div className="bg-[#E0E5EC] p-6 rounded-2xl shadow-[inset_4px_4px_8px_rgba(163,177,198,0.6),inset_-4px_-4px_8px_rgba(255,255,255,0.6)]">
                    <div className="flex items-center gap-4">
                      <div className="w-16 h-16 rounded-2xl flex items-center justify-center shadow-[4px_4px_8px_rgba(163,177,198,0.4),-4px_-4px_8px_rgba(255,255,255,0.4)]" style={{ backgroundColor: selectedColor }}>
                        <SelectedIconComponent size={32} className="text-white" />
                      </div>
                      <div>
                        <p className="text-[#3D4852] text-lg">{categoryName || 'Category Name'}</p>
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
              <NeumorphicCard className="p-6">
                ...
                {incomeCategories.length === 0 ? (
                  <p className="text-center text-[#8B92A0] py-8">No income categories yet</p>
                ) : (
                  <div className="grid grid-cols-2 gap-4">
                      ...
                      return (
                        ...
                        >
                          <div className="flex items-center gap-4 mb-4">
                            ...
                            <p className="text-[#3D4852] text-lg flex-1">{category.name}</p>
                          </div>
                          <div className="flex gap-2">
                            <button onClick={() => handleEdit(category)} className="flex-1 py-3 bg-[#E0E5EC] rounded-xl shadow-[3px_3px_6px_rgba(163,177,198,0.4),-3px_-3px_6px_rgba(255,255,255,0.4)] hover:shadow-[2px_2px_4px_rgba(163,177,198,0.3),-2px_-2px_4px_rgba(255,255,255,0.3)] transition-all flex items-center justify-center gap-2 text-[#6C63FF]">
                              <Edit2 size={16} /><span className="text-sm">Edit</span>
                            </button>
                            <button onClick={() => handleDelete(category.id)} className="flex-1 py-3 bg-[#E0E5EC] rounded-xl shadow-[3px_3px_6px_rgba(163,177,198,0.4),-3px_-3px_6px_rgba(255,255,255,0.4)] hover:shadow-[2px_2px_4px_rgba(163,177,198,0.3),-2px_-2px_4px_rgba(255,255,255,0.3)] transition-all flex items-center justify-center gap-2 text-[#FF6B6B]">
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}>
              <NeumorphicCard className="p-6">
                ...
                {expenseCategories.length === 0 ? (
                  <p className="text-center text-[#8B92A0] py-8">No expense categories yet</p>
                ) : (
                  <div className="grid grid-cols-2 gap-4">
